...
glimpse(crooked_river) # note date column is imported as a character (<chr>)
glimpse(woody_island) # note date column is imported as a character (<chr>)
```
...
dates <- seq(
  from = lubridate::ymd_hms("2000-01-01 00:00:00"),
  to = lubridate::ymd_hms("2000-12-31 23:59:59"), 
  by = "30 min"
  )
...
# check it
tibble::glimpse(fake_data) # info about temp.test
...
You can check whether your data is complete and continuous using the `check_continuous()` function. This function will return a message if the data is or is not continuous or complete. 
