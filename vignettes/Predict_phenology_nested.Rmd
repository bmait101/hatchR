...
**CURRENTLY NOT RENDERED WITH CODE - MUST UPLOAD DATAFILE**
...
```{r}
# isaak_dat <- read_csv("C:/Users/BryanMaitland/OneDrive - USDA/Projects/hatchR/data/Isaak_et_al_2018_long.csv")
# 
# isaak_dat <- isaak_dat |> na.omit()

#load rda file (because it is a very large datafile)
load(here::here("data-raw", "Isaak_et_al_2018.rda"))
...
PIBO_1345 <- isaak_dat_bt |> filter(site == "PIBO_1345")
...
PIBO_1345 |> arrange(SampleDate)
# looks like there are multiple records per day 
# so we need to summarize those in the larger dataset (isaak_dat_bt)
...
isaak_summ_bt |> purrr::pluck("summ_obj", 1)
```
...
```{r}
isaak_summ_bt |> tidyr::unnest(cols = summ_obj)
```
...
hatch_res |>
  ...
  ggplot(aes(x = days_to_hatch, y = spawn_time, fill = spawn_time, color = spawn_time)) + 
