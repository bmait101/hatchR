...
This vignette describes the process of parameterizing models for predicting fish phenology using `hatchR`. The package provides two options for choosing phenological models. One is to use model parameterizations installed the package and the other is to create your own using data you import. 
...
The vast majority of the time, what you will want is the actual expression for use in the `model = ...` argument of `predict_phenology()`. This is stored in the `func` element of the list, which is a tibble giving the species, development type, and the model expression. 

You can either pass this object directly with the `$` operator by calling `$func` element of the list (e.g., `model = bt_hatch_mod$func)`) or set as an object to pass, like so

```{r, eval=FALSE}
bt_hatch_exp <- bt_hatch_mod$func
bt_hatch_exp
```

`predict_phenology()` will extract the expression from the object and use it to predict phenology. We will demonstrate this in the **Predicting Phenology** vignettes.
