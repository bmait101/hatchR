...
The models presented in **hatchR** can be further customized beyond the use-cases provided above. For instance, while our models as presented are fit to predict hatch or emergence timing, they could be used to predict other developmental stages prior to the initation of external feeding like eye-up or embryological developmental stages @velsen1980 or **Some example from a non-salmonid, maybe initiation of pelagic-larval or riverine current dispersal**. Additionally, while `fit_model()` relies on non-linear modeling platform, `predict_phenology()` only requires users to pass a model expression, so if users preferred a different model fit than the non-linear option provided, they could pass a different expression into `predict_phenology()` for additional customization. Finally, while **hatchR** was designed specifically for fishes, we expect other poikilothermic organisms such as reptiles, amphibians, or invertebrates would develop in an exponential decay relationship and these developmental models could be extended to other organisms beyond fishes.
